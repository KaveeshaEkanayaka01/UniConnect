...
export default function ProjectFeed() {
  ...
  return (
    <div className="min-h-screen pb-50 bg-slate-75">
      ...
      <div className="relative py-8 px-4 text-center overflow-hidden border-b border-slate-200 bg-white">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-900 mb-4">
            Project <span className="gradient-text">Feed</span>
          </h1>
          <p className="text-slate-500 max-w-xl mx-auto text-lg mb-0">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-10">
        ...
        <div className="flex items-center gap-3 mb-6">
          {['All','Planned','Ongoing','Completed'].map((s) => (
            <button
              ...
              className={`px-3 py-2 text-sm rounded-full font-medium ${statusFilter===s ? 'bg-indigo-600 text-white' : 'bg-white text-slate-700 border border-slate-200'}`}
            >
              {s}
            </button>
          ))}
        ...
        {!loading && filtered.length === 0 && (
          <div className="glass-card p-16 text-center bg-white shadow-sm">
            <div className="text-5xl mb-4">🚀</div>
            <h2 className="text-slate-800 font-semibold text-xl mb-2">No Projects Yet</h2>
            <p className="text-slate-500 mb-6">Be the first to share a project with the community!</p>
            <button onClick={() => navigate("/UploadProject")} className="glow-btn px-8 py-3 rounded-xl font-semibold shadow-md">
              Upload Now
            </button>
          </div>
        )}
